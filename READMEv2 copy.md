...
- CUDA now performs both kmer hashing and HD encode on GPU (existing `.sketch` and `.ull` format preserved)
...
The CPU HD encoder builds a signed count hypervector from the deduplicated
sampled hashes:

- initialize every coordinate to `-N`, where `N` is the number of unique sampled
  hashes
- for each hash, generate deterministic random bits
- for each random `1` bit, add `2` to that coordinate

The CUDA HD encoder keeps the same math but performs the counting on GPU. The
CUDA sketch path still performs ULL updates and `HashSet` dedup/filtering on
host, then sends the unique sampled hash vector to GPU for HD encoding. The GPU
returns the same `Vec<i32>` hypervector that the existing norm, compression, and
serialization code already expects.

The key implementation detail is direct random access to the random word for a
given `(hash, 64-coordinate chunk)`. Instead of advancing a serial RNG stream on
CPU, the CUDA kernel computes the needed WyRng-equivalent word directly on GPU.
Each CUDA block owns one 64-coordinate chunk and one tile of hashes, uses
warp-level ballot/popcount-style counting to accumulate bit counts, then updates
the signed count vector. This preserves exact output compatibility while moving
the expensive HD encode stage off CPU.

This design was inspired by HyperSpec's GPU execution pattern: HD encoding can
be expressed as many coordinate-level reductions over input features. In
HyperSpec the features are mass-spectrometry peaks and the output is a packed
binary hypervector; in dotANI the features are unique sampled k-mer hashes and
the output remains a signed `i32` count hypervector. We borrowed the GPU
parallelization shape, not HyperSpec's data model, packed binary format, or
