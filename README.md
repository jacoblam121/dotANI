...
_dotANI_ first samples the kmer set using dotHash. Then the kmer hashes are encoded into hyperdimensional vectors (HVs) using HDC encoding to obtain better tradeoff of ANI estimation quality, sketch size, and computation speed. The sketch size generated by _dotANI__ is 1.8 to 2.7× smaller than _Mash_ and _Dashing 2_. ANI estimation in _dotANI__ can be realized using highly vectorized vector multiplication. _dotANI__'s database search speed for large-scale datasets is up to 4.3x faster than _Dashing 2_.
...
#### Basic Installation
_dotANI__ requires [`Rust`](https://www.rust-lang.org/tools/install) language and [`Cargo`](https://doc.rust-lang.org/cargo/) to be installed. 
