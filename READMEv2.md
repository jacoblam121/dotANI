...
The most important finding is that the current CUDA path is still primarily
limited by CPU-side post-hash work, not by CUDA transfer or launch overhead.

Current CUDA path is still limited by CPU pipeline
